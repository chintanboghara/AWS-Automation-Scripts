...
def cleanup_old_lambda_versions(function_name: str, dry_run: bool = False) -> None:
    ...
    every version except '$LATEST'. The dry-run mode allows you to simulate deletions
    without making any changes.

    Args:
        function_name (str): The name or ARN of the Lambda function.
        dry_run (bool): If True, simulate deletion without actually deleting versions.
    """
    ...
    if not versions:
        print("No versions found for the Lambda function.")
        return
    ...
    if not deletable_versions:
        print("No old versions found for deletion (only '$LATEST' is present).")
        return
